...
class TestFiatToXMRSync:
    ...
    def test_fiat_to_xmr_sync_no_cache_running_loop(self):
        ...
        # Mock fetch_xmr_rates to avoid external API call
        with patch('bot.services.currency.fetch_xmr_rates', new_callable=AsyncMock) as mock_fetch:
            mock_fetch.return_value = {"USD": Decimal("150.00"), "GBP": Decimal("120.00"), "EUR": Decimal("140.00")}
            result = fiat_to_xmr_sync(Decimal("100.00"), "USD")
            assert isinstance(result, Decimal)
