...
class TestMain:
    ...
    @patch('bot.main.PostageService')
    @patch('bot.main.PayoutService')
    @patch('bot.main.HealthCheckServer')
    @patch('bot.main.ApplicationBuilder')
    def test_build_app(self, mock_app_builder, mock_health, mock_payout, mock_postage,
                       mock_orders, mock_payments, mock_catalog, mock_vendors,
                       mock_db, mock_setup_logging, mock_settings):
        ...
        mock_postage.assert_called_once()
        ...
        # 13 commands + 12 callback handlers + 1 message handler = 26
        assert mock_app.add_handler.call_count == 26
        mock_app.add_error_handler.assert_called_once()

        # Verify bot_data set
        assert "db" in app.bot_data
        assert "health_server" in app.bot_data
        assert "postage" in app.bot_data
        assert "payout_service" in app.bot_data
