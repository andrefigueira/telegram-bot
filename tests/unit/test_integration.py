...
class TestIntegration:
    ...
    def test_full_order_flow(self, db, test_settings):
        ...
        vendor = vendors.add_vendor(
            Vendor(telegram_id=123456789, name="Test Store")
        )
        ...
        assert order_data["total_xmr"] == 3.0  # 1.5 * 2
        ...
        assert order.commission_xmr == 0.15  # 3.0 * 0.05
        ...
        # Mark as paid
        paid_order = orders.mark_paid(order.id)
        assert paid_order.state == "PAID"
        
        # Fulfill order
        fulfilled_order = orders.fulfill_order(order.id)
        assert fulfilled_order.state == "FULFILLED"
    ...
    def test_vendor_commission_flow(self, db, test_settings):
        ...
        # Create vendor with custom commission
        vendor = vendors.add_vendor(
            Vendor(telegram_id=111222333, name="Premium Store")
        )
        
        # Set custom commission rate
        updated_vendor = vendors.set_commission(vendor.id, 0.10)
        assert updated_vendor.commission_rate == 0.10
        ...
        # Verify commission
        order = orders.get_order(order_data["order_id"])
        assert order.commission_xmr == 1.0  # 10.0 * 0.10
    ...
    def test_inventory_management(self, db, test_settings):
        ...
        # Setup
        vendor = vendors.add_vendor(Vendor(telegram_id=123, name="Store"))
        product = catalog.add_product(
            Product(
